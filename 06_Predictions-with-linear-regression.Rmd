...
As usual we first always check and load in our required packages `here` and `tidyverse`.
...
Before we begin coding, let's think about our research question. We learned last week that there is a statistical relationship between political attitude and time spent on social media. This tells us that we should be able to somewhat predict time spent on social media from political attitude.
...
> Write in the comments of your code whether you expect a positive or negative relationship, and if you are feeling statistically powerful, what proportion of variance of time spent on social media you would expect to be accounted for by political attitude.
...
First, we need to load our data set for today. This week we will be using the  `PSYC2001-social-media-attitude` data set which includes the `attitude` variable we created last week.  
...
Now, this is no good, as both the `polit_attitude` and `age` values are in one plot. We need to separate them! Luckily, there is a very hand function called `facet_wrap()` that will do this for you.
...
::: {style="border-left: 4px solid #33a02c; background-color: #a6cee3; padding: 10px; margin: 10px 0;"}
<strong>Question:</strong> Would you say the variables are normally distributed? Are you happy the data have met the assumptions for the regression analysis?
:::

## Linear regression in R 

The difference between a regression and correlation is that instead of looking for a statistical relationship between $x$ and $y$, we are asking how well we can predict $y$ on the basis of $x$. Like if we were stuck on a desert island with only $x$, how much would we know about what $y$ is doing? 
...
The first argument is `formula`, which is where we specify the dependent and independent variables in our regression. We have already used formulas in Sections \@ref(sec-formula-for-independence) and \@ref(sec-cor-form). In fact, the formula we need looks very similar to the one in Section \@ref(sec-formula-for-independence), except that we now have a different DV, and a continuous predictor rather than an IV.

> Complete the following comment in your code with the formula that you will need to predict the time spent on social media from political attitude.
...
mod <-lm(formula = ... ~ ..., data = ... ) # time_on_social is on the left side of the tilde as it is being predicted by attitude 
```
...
> Run the following line of code in your script to see the output of the linear regression function.
...
After that, there are two coefficients output. The first is the Intercept (which we have referred to as $a$ in the lectures). The second is the sloped of the regression line (which we referred to as $\mathrm{Beta}$ in the lectures).

> Use the two co-efficients to write a regression equation that predicts time spent on social media on the basis of political attitude in the comments in your code. Is the regression co-efficient positive or negative? Is this what you expected?
...
## ⭐ Bonus exercises -  level up plots with Gen AI

### Bonus exercise - Tidy up the regression plot

I don't know about you, but the default colours for the line of best fit supplied by `geom_smooth()` are quite awful on the eye. You may have noticed from the help documentation that it's not super clear how we would get rid of this smite on the eyes. Let's see if a Gen AI model can help us level up our plot.

> Using your favourite LLM (like ChatGPT or Copilot), enter the prompt that you are using `geom_smooth()` and that you want to change the colour of the line of best fit. Can you follow what the model tells you to update your code and change the colours?

### Bonus exercise - to the movies!

We have previously used `theme_classic()` to tidy up our plots and make them look neat. But why have your figure look neat when it can look like it came from a movie? There are a plethora of movie themes that you can use to boost your plots, using the [ThemePark](https://github.com/MatthewBJane/ThemePark) package. Just like in Section \@ref(sec-unleash-cats), we need to install the ThemePark package using the `remotes` package.

> Add the following code to the top of your script and run it.

```{r, install ThemePark, warning=FALSE, message=FALSE, eval=FALSE}
...
The below code is an example from the [ThemePark Github Page](https://github.com/MatthewBJane/ThemePark). 

> Can you amend your own code for your density plot to use the movie theme of your choice(s)?

```{r, example, eval=FALSE}

ggplot(mpg, aes(cty)) + geom_density(fill = barbie_theme_colors["medium"]) +
    labs(
        title = "Density plot",
...
```{r, do it, echo = FALSE, message=FALSE}
