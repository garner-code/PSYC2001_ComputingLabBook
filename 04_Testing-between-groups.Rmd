...
Just like in Chapter \@ref(sec-testing-first-hypothesis), your script contains the code to install the packages, if required. Add the two lines of code that use the `library()` function to load `here` and `tidyverse`. Run all the code.
...
Looking at the data frame above, we can see that the first value for `good_mood_likes` is 22.8 and the first value for `bad_mood_likes` is 46.5. Lets add them together and divide by 2, to check the answer is the same as the first value for `likes`, which is 34.65.
...
Now, can you make a density plot for `followers`? Copy and paste the code you used to make the histogram for `followers` and make the teeny changes required. Go on, you are a coder, and you are strong.
...
However, we would reconsider this choice if `followers` contained a massive outlier, such as if we sampled someone with 10 million followers. 
...
This is how we use the formula method in `t.test()`. The syntax is `t.test(DV ~ group, data = data frame)`. So, for our data, we want to test if the dependent variable (DV) `likes` differs by the groups coded in `urban` , using the `social_media_likes` data frame. 
...
<strong>Results:</strong> An independent samples t-test indicated that the number of likes was greater for rural (<i>M</i> = 52.1 , <i>SD</i> = 12.7) compared to urban dwellers  (<i>M</i> = 40.8 , <i>SD</i> = 14.5) and this difference was statistically significant (<i>t</i>(58) = 3.22, <i>p</i> = 0.002). Conversely, the number of followers was greater for urban (<i>M</i> = 143.9, <i>SD</i> = 62.0) than for urban dwellers (<i>M</i> = 105.6, <i>SD</i> = 40.9) which was statistically significant (<i>t</i>(58) = 2.82, <i>p</i> = 0.007).
:::
