...
### Activity - How might political attitudes relate to social media use? Get Data
...
> Examine the data frame using the head() function (or using another method if you prefer). Take the first 3 observations from `polit_informed` and multiply each of them by 0.25 (in the console, or in your head). What are the 3 results that you get? Write them as a comment in your script.
...
Looking for lines {#sec-look-lines}
...
Would you expect the correlation between political attitude and time spent on social media to be the same for urban and rural participants?

> Can you use the `filter()` function to create two new data frames: one containing only urban participants, and containing only rural participants? Then run correlation analyses from this tutorial, using your new data frames, to see if the correlation coefficients differ between the two groups.

Also, remember to visualise the data for each group!

### Bonus Activity - Piping out a summary of correlations for Urban vs Rural groups

It is possible to get the correlation values for Urban vs Rural groups, by adding to the code from the previous activity.

> Can you amend your code from the previous activity to use the `group_by()` and `summarise()` functions to build a set of pipes where you take the `social_media_attitude` data frame, and the result is separate correlation coefficient values between the `time_on_social` and `age` variables for Urban and Rural participants?

Check how you previously used the `summarise()` and `group_by()` functions for clues!

### Bonus Activity - Correlation Matrix

Sometimes it`s helpful to see all the correlations you are interested in at once. 

> Can you use the `select()` and [`cor()`](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/cor) function to create a pipe that takes the `social_media_attitude` data frame, and that produces a [correlation matrix](https://www.displayr.com/what-is-a-correlation-matrix/) that shows the correlations between `time_on_social`, `polit_attitude`, and `age`?
