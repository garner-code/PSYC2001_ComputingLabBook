...
We are not giving you the code to copy and paste. You are ready to fly, and write it yourself, now you have mastery of the codeship. You are welcome to decide yourself whether you would like to check if the packages are installed before you load them.
...
Now that you have loaded the data, what do you think about the relationship between political attitudes and time spent on social media? Will young adults who spend a lot of time on social media be more politically active? Or the opposite? Or neither? 
...
Now you have this formula, are you able to figure out how to write the code that creates the political attitude scores? 

### Activity - putting the formula together in code form
...
That's one way to see the observations in a variable. The second way is to use the `select` function from tidyverse. (There are always many ways to solve problems when coding, and knowing the different methods is useful, as different solutions will be more or less useful, depending on the context of what you are doing). 

Run the following line of code in your script, and check you see the same numbers as before. Note that the numbers will be printed out in a tibble format, as tidyverse has automatically kept the data frame structure when using the `select()` function.

```{r, access numbers using select function, eval=FALSE}

social_media %>% select(polit_informed)

```

Now, to make things neater, let's just look at the first 5 observations of this variable. In your own script, complete the line of code you see below so that you pipe the output of the `select()` function to be used as input to the `head()` function. 

```{r, access first 5 numbers using head function, eval=FALSE}

social_media %>% select(polit_informed) %>%

```

**Note that** if you look up the help for the `head()` function by typing `?head` into the console, you will see that the `head()` function returns the first or last parts of a vector (variable), matrix, table, data frame or function. The thing that `head()` is returning the first or last parts of is called `x` in the help file. Piping the output of the `select()` function to the `head()` function means that the output will automatically be used as the input `x` to the `head()` function. Thus you will just be able to use `head()` without specifying what `x` is - aka you do not need to write anything between the brackets.

Run your completed line of code and make sure that the 6 numbers output into the console match the first 6 numbers that you saw when you ran `social_media$polit_informed` and `social_media %>% select(polit_informed)`. 


```{r, access first 5 numbers for real, echo=FALSE}

social_media %>% select(polit_informed) %>% head()

```

If you wanted, you could get a similar result by using `head(social_media$polit_informed)`, but without the tibble/data frame bells and whistles that come with using `select()`.

OK, now we know we are accessing the 
