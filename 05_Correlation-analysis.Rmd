...
> Can you use the `filter()` function to create two new data frames: one for urban participants, and another for rural participants? Then run correlation analyses from this tutorial on both data frames to see if the correlations differ between the two groups.
...
> Can you amend your code from the previous activity to use the `group_by()` and `summarise()` functions to build a set of pipes where you take the `social_media_attitude` data frame, and the result is separate correlation values between the `time_on_social` and `age` variables for Urban and Rural participants?
