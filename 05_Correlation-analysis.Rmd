...
> Run the code to make this scatterplot of the relationship between `time_on_social` and `attitude`. 
...
social_media_attitude %>% 
  ggplot(aes(x = time_on_social, y = polit_attitude)) + # set up the canvas
  geom_point(colour = "orange") + # make a scatterplot
  labs(x = "Time on Social", y = "Political Attitude") + # define labels
  theme_classic() # make pretty

```

::: {style="border-left: 4px solid #33a02c; background-color: #a6cee3; padding: 10px; margin: 10px 0;"}
<strong>What do you think?</strong> Would you say there is a linear relationship between `time_on_social` and `attiutude`?
:::
...
> Now, using the above code as a guide, create two more scatterplots: one of the relationship between `age` and `attitude`, and another of `time_on_social` and `attitude`age`.
...
### Last bonus exercise - unleash the cats!
